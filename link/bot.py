...
async def require_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    logger.warning("Unauthorized command from %s", update.effective_user.id if update.effective_user else "?")
    return False
...
async def discord_send_message(content: str):
    ...
    while True:
        tries += 1
        logger.info("Discord POST try=%d len=%d", tries, len(content))
        async with aiohttp.ClientSession() as sess:
            async with sess.post(url, json={"content": content},
                ...
                if resp.status == 429:
                    ...
                    logger.warning("Discord 429, retry_after=%s", retry_after)
                    await asyncio.sleep(retry_after); continue
                if 500 <= resp.status < 600 and tries < 3:
                    logger.warning("Discord %s, retrying…", resp.status)
...
async def _notify_finished_and_stop(app: Application):
    ...
    logger.info("All items sent. Scheduler stopped.")

async def scheduler_loop():
    logger.info("Scheduler START interval_ms=%d", STATE.interval_ms)
    await process_tick()
...
def set_interval_ms(ms: int):
    ...
    if STATE.running:
        STATE.next_send_at = datetime.now(timezone.utc) + timedelta(milliseconds=STATE.interval_ms)
    logger.info("Interval updated to %d ms", ms)
...
def main_menu_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        [[BTN_LINK],[BTN_START, BTN_STOP],[BTN_SET_INTERVAL, BTN_LIST]],
...
def build_watch_text() -> str:
    ...
    next_name = q[0]["url"] if q else "(kosong)"
    return (
        f"{spin} Bot Link Status\n"
...
async def cmd_start_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    q = load_queue()
    if not q:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="Antrian kosong. Tambahkan link dulu.")
        return
